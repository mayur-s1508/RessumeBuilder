...
export default function Por() {
  ...
  const save = (showProgress, newData, callback = () => {}) => {
    ...
    firestore()
      ...
      .set(
        {
          por: newData.map((d) => ({ ...d, tableData: null })),
        },
        { merge: true }
      )
      .then(() => {
  ...
  React.useEffect(() => {
    ...
    firestore()
      ...
      .get()
      .then((doc) => {
        if (doc.exists && doc.data().por !== undefined) {
          setData(doc.data().por);
        }
        setPending(false);
      })
      .catch((e) => {
  ...
  return (
    <div>
      ...
      <BaseForm data={data} save={save} headline="Position of Responsibility" />
