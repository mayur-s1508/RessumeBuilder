...
export default function Activities() {
  ...
  const save = (showProgress, newData, callback = () => {}) => {
    ...
    firestore()
      ...
      .set(
        {
          activities: newData.map((d) => ({ ...d, tableData: null })),
        },
        { merge: true }
      )
      .then(() => {
  ...
  React.useEffect(() => {
    ...
    firestore()
      ...
      .get()
      .then((doc) => {
        if (doc.exists && doc.data().activities !== undefined) {
          setData(doc.data().activities);
        }
        setPending(false);
      })
      .catch((e) => {
  ...
  return (
    <div>
      ...
      <BaseForm
        data={data}
        save={save}
        headline="Participated Activities &amp; Achievements"
      />
